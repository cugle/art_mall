...
if( $_POST['action'] == 'edit' && $_POST['scid'] > 0){ 
		$sql = "UPDATE " . $db_table . " SET  descs='$descs' WHERE  scid=".$Aconf['domain_id'];
		$oPub->query($sql); 
		/* 处理相册图片 */
		/* 编辑图片描述 old_img_desc */
		...
		$type='about';$arid =$Aconf['domain_id'];
		handle_gallery_image($arid, $_FILES[img_url], $namedesc,$type); 
		$strMessage = '公告编辑成功'; 
}
/* 网站配置信息 */
 
$sql = "SELECT scid,descs FROM ".$pre."sysconfig WHERE scid = ".$Aconf['domain_id']." ORDER BY scid ASC LIMIT 1";
$Anorm = $oPub->getRow($sql);  
$sql = "SELECT * FROM " . $pre."arti_file WHERE type = 'about' and domain_id=".$Aconf['domain_id'];
...
<TABLE id=gallery-table width="100%" align=center cellspacing="0" cellpadding="0">
 <tr>
  ...
  <td>
		...
		<input type="submit" name="Submit" value="<?php echo ($Anorm['scid'])?'编辑':'增加';?>" style="margin-left:80px;background-color: #FFCC66"/>
...
//<![CDATA[ 
  ...
  {
    obj = "delimg_show";
	var strTemp = "ajax_arti_delimg.php?fileid=" + fileid +   "&op=delimg&action=about";  
...
<script>
	...
	KindEditor.ready(function(K) {  
		editor = K.create('textarea[name="descs"]', {
			...
			uploadJson : '../upload_json.php?jsonop=about',
			fileManagerJson : '../upload_manager_json.php?jsonop=about',
