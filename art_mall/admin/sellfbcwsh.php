...
if($_POST['act'] == 'update' &&  $_POST["id"] > 0) {  
	...
	if($_POST['shenhecwstats']>0  ){
		$Afields=array('shenhecwstats'=>$_POST['shenhecwstats'], 'shenhecwuserid'=>$_SESSION['auser_id'],'shenhecwname'=>$_SESSION[$un_auser_name],'shenhecwdescs'=>$_POST['shenhecwdescs'],'shenhecwdate'=>date("Y-m-d H:i:s")); 
		$condition = 'id='.$_POST["id"].' and shenhestats=2 and chunastatsuserid<1  AND domain_id = '.$Aconf['domain_id']; 
...
//列表
$where = " domain_id = '".$Aconf['domain_id']."'  and shenhestats=2 "; 
...
$sql = "SELECT * FROM ".$pre."feibao  WHERE  $where ".
       " ORDER BY shenhecwstats asc, dateadd DESC ".
       " LIMIT ". $strOffSet;
$row = $oPub->select($sql);
if($row ) { 
...
foreach ($row AS $key=>$val)
{
	...
	$strstates = '';
	   ...
	   $StrtypeAll .= '<TD align=middle ><a title="'.$states .'">'.$strstates.'</a></TD>'; 
        ...
        if($val["chunastatsuserid"] < 1 && $val["shenhestats"] == 2){
...
<TABLE width="100%" border=0 cellspacing="1" cellpadding="0">
  ...
  <TR class=bg1>
    <TD align=left > 
		...
		<?php if($work["chunastatsuserid"] < 1 && $work["shenhestats"] == 2){?>
		  <TR>
			<TD width="120" align="right"><b>董事审核:</b></TD>
			<TD>
		    <INPUT TYPE="radio" NAME="shenhecwstats" value="1" <?php echo $work["shenhecwstats"]==1?"checked":'';?>>不通过	
			<INPUT TYPE="radio" NAME="shenhecwstats" value="2" <?php echo $work["shenhecwstats"]==2?"checked":'';?>>通过
			<INPUT TYPE="radio" NAME="shenhecwstats" value="0" <?php echo $work["shenhecwstats"]<1?"checked":'';?>>不处理
			</TD>
		  </TR>  		  
		  <TR>
			<TD width="120" align="right"><b>备注说明:</b></TD>
			<TD>
		    	  <TEXTAREA NAME="shenhecwdescs" style="height:40px;width:400px;background-color: #FFFFD0"><?php echo ($work["id"] > 0)?$work["shenhecwdescs"]:'';?></TEXTAREA>
			</TD>
		  </TR>
		  <TR>
