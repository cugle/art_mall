...
{
    	/*处理图片*/
	if($_FILES['pro_interval']['size'] > 0 )
	{
		/* 判断图像类型 */
        if (!$image->check_img_type($_FILES['pro_interval']['type']))
        {
            $strMessage =  '图片类型错误';
			$img_name = $_POST['old_pro_interval'];
        }
		else
		{

	       if(!empty($_POST['old_pro_interval']))
	       {
               $img_name = basename($image->upload_image($_FILES['pro_interval'],'brandlogo',$_POST['old_pro_interval']));
	       }
	       else
	       {
		  
		       $img_name = basename($image->upload_image($_FILES['pro_interval'],'brandlogo'));
	...
	$Afields=array('fid'=>$_POST['fid'],'next_node'=>'','name'=>$_POST['name'],'descs'=>$_POST['descs'],'ifnav'=>$_POST['ifnav'],'ifhot'=>$_POST['ifhot'],'pro_interval'=>$img_name,'acids'=>$acids,'domain_id'=>$Aconf['domain_id']);
...
if( $_POST['action'] == 'edit' && $_POST['pcid'] != $_POST['fid']){
	...
	$Afields=array('fid'=>$_POST['fid'],'name'=>$_POST['name'],'descs'=>$_POST['descs'],'ifnav'=>$_POST['ifnav'],'ifhot'=>$_POST['ifhot'],'pro_interval'=>$img_name,'acids'=>$acids);
...
<TABLE width="100%" border=0>
  ...
  <form name="form1" method="post" enctype="multipart/form-data" action="<?php echo $_SERVER["PHP_SELF"]?>"> 
    <TD width="13%" align="left" colspan="7">
		...
		<INPUT type="file" name="pro_interval" style="width:140px" /><?php if($Anorm['pro_interval']&&$Anorm['pcid']) {?> <a href="/data/brandlogo/<?php echo $Anorm['pro_interval']?>" target="_blank"><img src="images/icon_view.gif" width="16" height="16" border="0" alt="显示缩图"></a> <?php }?><br/>
		(注：图片尺寸为 125*90px 支持：.jpg .gif .png 格式，)
		<INPUT type="hidden" name="old_pro_interval"  value="<?php echo ($Anorm['pcid'])?$Anorm['pro_interval']:'';?>" />
		<br/>
