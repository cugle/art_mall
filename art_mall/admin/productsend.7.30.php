...
if($act == 'insert' || $act == 'update' ) {
    ...
    if(trim($_POST[name]) == '' ) {
	  ...
	  if($is_insert)
	  {
		...
		$Afields=array('pcid'=>$pcid,'pacid'=>$pacid,'prbid'=>$prbid,'user_id'=>$myuser_id,'name'=>$name,'enname'=>$enname,'edit_comm'=>$edit_comm,'praids'=>$praids,'shop_sn'=>$shop_sn,'shop_price'=>$shop_price,'up_date'=>$up_date,'shop_number'=>$shop_number,'s_discount'=>$s_discount,'s_dis_exp'=>$s_dis_exp,'min_thumb'=>$min_thumb,'shop_thumb'=>$shop_thumb,'top'=>$top,'special'=>$special,'colors'=>$colors,'dateadd'=>gmtime(),'states'=>$states,'domain_id'=>$Aconf['domain_id']);
