...
if($_POST['act'] == 'update' &&  $_POST["id"] > 0) {  
	...
	if($_POST['shennwstats']>0  ){
		$Afields=array('shennwstats'=>$_POST['shennwstats'], 'shennwuserid'=>$_SESSION['auser_id'],'shennwname'=>$_SESSION[$un_auser_name],'shennwdescs'=>$_POST['shennwdescs'],'shennwdate'=>date("Y-m-d H:i:s")); 
		$condition = 'id='.$_POST["id"].' and shenhestats<1  and chunastatsuserid<1 and shenhecwstats<1 AND domain_id = '.$Aconf['domain_id']; 
...
if(  $_GET['action'] == 're'){ 
	$id = $_REQUEST[id] + 0;
	$sql = "SELECT a.* FROM ".$pre."feibao  as a 
			where  a.id = '$id' AND a.domain_id=".$Aconf['domain_id'];
    $work = $oPub->getRow($sql);  
}
 
 
//列表
$where = " domain_id = '".$Aconf['domain_id']."' and prestats=2"; 
...
$sql = "SELECT * FROM ".$pre."feibao  WHERE  $where ".
       " ORDER BY shennwstats asc,dateadd DESC ".
       " LIMIT ". $strOffSet;
$row = $oPub->select($sql);
if($row ) { 
...
foreach ($row AS $key=>$val)
{
	  ...
	  $strstates = '';
	   ...
	   $StrtypeAll .= '<TD align=middle ><a title="'.$states .'">'.$strstates.'</a></TD>'; 
        ...
        if($val["prestats"] == 2 && $val["shenhestats"] < 1){
    ...
    <TD align=left > 
		  ...
		  <TR>
			...
			<TD>
				...
				&nbsp;&nbsp;<b>金额:</b> <span style="color:#0000ff"><?php echo ($work["id"] > 0)?$work["money"].'元':'';?></span> 
			</TD>
		  </TR>
		  <TR>
		  ...
		  <TR>
			...
			<?php } else {?>
						<INPUT TYPE="radio" NAME="shennwstats" value="1" <?php echo $work["shennwstats"]==1?"checked":'';?>>不通过	
						<INPUT TYPE="radio" NAME="shennwstats" value="2" <?php echo $work["shennwstats"]==2?"checked":'';?>>通过
						<INPUT TYPE="radio" NAME="shennwstats" value="0" <?php echo $work["shennwstats"]<1?"checked":'';?>>不处理
			<?php } ?>
			</TD>
		  </TR>  


			<?php if($work["shenhestats"] < 1){ ?>		  
		  <TR>
			<TD width="120" align="right"><b>备注说明:</b></TD>
			<TD>

		    	  <TEXTAREA NAME="shennwdescs" style="height:40px;width:400px;background-color: #FFFFD0"><?php echo ($work["id"] > 0)?$work["shennwdescs"]:'';?></TEXTAREA>
			</TD>
		  </TR> 
		  <TR>
