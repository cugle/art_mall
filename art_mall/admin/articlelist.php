...
foreach ($row AS $key=>$val)
{
	   ...
	   $StrtypeAll .= '</TD>';
       $StrtypeAll .= '<TD align=left>';
	   $StrtypeAll .= '<a href="../article.php?id='.$val["arid"].'" target="_blank"><IMG SRC="images/icon_view.gif" WIDTH="16" HEIGHT="16" BORDER="0" ALT="阅读"></a> ';
...
$StrtypeAll .= '<td></td>';
$StrtypeAll .= '<td>';
$StrtypeAll .= '<B>合计totle</B>:'.$Asum["sum_count"];
$StrtypeAll .= '</td>';
/*$StrtypeAll .= '<td></td>';
$StrtypeAll .= '<td></td>';
$StrtypeAll .= '<td></td>';*/
  ...
  <TR class=bg1>
	...
	<FORM METHOD=POST ACTION="" style="margin: 0px">
	   ...
	   <?php echo $Stropt;?> 
		...
		<input name="selbtn1" id="selbtn1" onclick="return showCalendar('start_time', '%Y-%m-%d', false, false, 'selbtn1');" value="选择select" type="button" >
		<?php
			if(!empty($end_time)){
				$end_time  =time();
			}
		?>
        结束日期end date<input style="width: 68px" name="end_time" id="end_time"  value="<?php echo !empty($end_time)?date("Y-m-d",$end_time):date("Y-m-d");?>" readonly="readonly" type="text">
		<input name="selbtn2" id="selbtn2" onclick="return showCalendar('end_time', '%Y-%m-%d', false, false, 'selbtn2');" value="选择 select"  type="button" >
		标题title<INPUT TYPE="text" NAME="sear_name" value="<?php echo $sear_name;?>" >
        <INPUT TYPE="submit" name="submit" value="确定查询submit">
		<INPUT TYPE="hidden" name="act" value="datesearch">
		<INPUT TYPE="hidden" name="action" value="">
       </span>
	   <span style="float: right"><a href="articlesend.php"> [添加新文章 add new]</a> </span>
	   </FORM>
	</TD> 
  </TR>
...
<form method="POST" action="<?php echo $PHP_SELF;?>" name="listForm" target="_self">
  <TR class=bg5>
	...
	<TD width="10%" align=left>操作operate</TD>
    <TD width="20%" align=left>标题title</TD>
	<TD width="9%" align=left>分类category</TD>
	...
	<a href="articlelist.php?acid=<?php echo $acid;?>&sort_by=arti_date&page=<?php echo $pagenew;?>&start_time=<?php echo $start_time;?>&end_time=<?php echo $end_time;?>" target="main">文章时间article date</a>
	</TD>
	...
	<a href="articlelist.php?acid=<?php echo $acid;?>&sort_by=dateadd&page=<?php echo $pagenew;?>&start_time=<?php echo $start_time;?>&end_time=<?php echo $end_time;?>" target="main">添加时间add time</a>
	</TD>
 ...
 <?php echo $StrtypeAll;?>
  ...
  <TR class=bg5>
	...
	<INPUT TYPE="submit" name="submit" value="确认删除delete" >
	<INPUT TYPE="reset" name="reset" value="恢复unselect"> 
	   ...
	   while( @list( $k, $v ) = @each( $Agrad ) ) {
           if ($v == 0 && $v =='')
		   {
			...
			if( $Anorm["name"] != ''){
	          ...
	          $Strtab .= '<TD align=left>'.$tmp.'</TD>';
              $Strtab  .= '<TD align=left><a href="'.$PHP_SELF.'?acid='.$v.'&fid='.$fid.'&action=edit"><IMG SRC="images/b_edit.png" WIDTH="16" HEIGHT="16" BORDER="0" ALT="[编辑edit]"></a> ';
	          $Strtab  .= '<a href="'.$PHP_SELF.'?acid='.$v.'&fid='.$fid.'&action=del" onclick="return(confirm(\'确定删除sure to delete?\'))"><IMG SRC="images/b_drop.png" WIDTH="16" HEIGHT="16" BORDER="0" ALT="[删除delete]" ></a></TD>';
              $Strtab  .= '</TR>';  
	          $n ++;
