...
if($_POST['act'] == 'insert' || $_POST['act'] == 'update' ) { 
	if($_POST["money"] < 1){
			$strMessage = 'COO还没确认，费用保险单！需要确认后才能审核！';
	}else{
		$sql = "SELECT pra_name  FROM ".$pre."pravail WHERE  praid=".$praid; 
		$pra_name = $oPub->getOne($sql);  
		
		$Afields=array('praid'=>$praid,'pra_name'=>$pra_name,'user_id'=>$_SESSION['auser_id'],'user_name'=>$_SESSION[$un_auser_name],'money'=>$_POST["money"],'descs'=>$_POST["descs"],'year'=>date("Y"),'month'=>date("n"),'dateadd'=>gmtime(),'domain_id'=>$Aconf['domain_id']);
...
foreach ($row AS $key=>$val)
{
       ...
       $StrtypeAll .= '<TD align=center>';
		...
		if( $val["prestats"]== 2 and $val["shennwstats"] < 1 && $val["shenhestats"] < 1 && $val["shenhecwstats"] < 1 && $val["chunastatsuserid"] < 1   ){
			//是否允许当天数据修改
			$StrtypeAll .= '<a href="'.$_SERVICE["PHP_SELF"].'?id='.$val["id"].'&action=edit&page='.$pagenew.'" target="main"><IMG SRC="images/b_edit.png" WIDTH="16" HEIGHT="16" BORDER="0" ALT="确认费用报销单"></a> ';  
		}else{
...
<?php if($_GET['action'] == 'show') {?> 
	<TABLE width="100%" border=0 cellspacing="1" cellpadding="0"> 
	  <TR class=bg1>
		<TD align=left > 
			  <span style="margin-left:120px;color: #FF6633;font-size: 14px;font-weight: bold">费用报销: </span> 
			  <TABLE width=96%>
			  ...
			  <TR>
				...
				<TD>
					  <span style="color:#0000ff"><?php echo ($work["id"] > 0)?$work["money"]:'';?> 元；</span> 
				</TD>
			  </TR> 
			  <TR>
	  ...
	  <TR class=bg1>
		<TD align=left > 
			  ...
			  <TR>
				<TD width="120" align="right"><b>申请金额</b></TD>
				<TD>
					  <span style="color:#00FF00"><?php echo ($work["id"] > 0)?$work["premoney"]:'';?>元</span>； 
				</TD>
			  </TR>
			  <TR>
				<TD width="120" align="right"><b>实报金额</b></TD>
				<TD>
					  <INPUT TYPE="text" NAME="money" value="<?php echo ($work["money"] > 0)?$work["money"]:$work["premoney"];?>" size="10">元；
