...
if( $_POST['action'] == 'edit' && $_POST['praid'] > 0){
        $sql = "UPDATE " . $db_table . " SET  descs='$_POST[descs]' WHERE  praid='".$_SESSION['apraid']."'";
        $oPub->query($sql);
		/* 编辑图片描述 old_img_desc */
		...
		$type='prav_about';$arid =$_SESSION['apraid'];
		handle_gallery_image($arid, $_FILES['img_url'], $_POST['namedesc'],$type); 
		$strMessage = '公告编辑成功';
}
/* 网站配置信息 */
$db_table = $pre."pravail";
$sql = "SELECT praid,pra_name,descs FROM ".$db_table." WHERE praid = '".$_SESSION['apraid']."' ORDER BY praid ASC LIMIT 1";
$Anorm = $oPub->getRow($sql); 
$sql = "SELECT * FROM " . $pre."arti_file WHERE type = 'prav_about' and arid=".$_SESSION['apraid'];
...
<TABLE width="800" border=0>
  <TR> 
    <TD align="left" > 
		<span style="font-weight:bold"><?php echo $Anorm['pra_name'];?> 简介：</span> 
	    <textarea name="descs" style="width:750px;height:400px;visibility:hidden;"><?php echo $Anorm['descs'];?></textarea>
    </TD>
  </TR>	
</TABLE>
<!-- 附件 start --> 
...
<TABLE id=gallery-table width="100%" align=center cellspacing="0" cellpadding="0">
 <tr>
  ...
  <TD>
	...
	图片描述:<INPUT TYPE="text" NAME=namedesc[] value="" size="60"/>
  ...
  <tr>
	<td>
        ...
        <input type="submit" name="Submit" value="<?php echo ($Anorm['praid'])?'编辑提交':'增加';?>" style="background-color: #FFCC66"/>
...
//<![CDATA[ 
  ...
  {
    obj = "delimg_show";
	var strTemp = "ajax_arti_delimg.php?fileid=" + fileid +  "&op=delimg&action=pra_descs";  
...
<script>
	...
	KindEditor.ready(function(K) {  
		editor = K.create('textarea[name="descs"]', {
			...
			uploadJson : '../upload_json.php?jsonop=prav_about&arid=<?php echo $_SESSION['apraid'];?>',
			fileManagerJson : '../upload_manager_json.php?jsonop=prav_about&arid=<?php echo $_SESSION['apraid'];?>',
