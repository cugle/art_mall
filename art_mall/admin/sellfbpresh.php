...
if($_POST['act'] == 'update' &&  $_POST["id"] > 0) {  
	//prestats predate preuserid prename predescs
	
	if($_POST['prestats']>0  ){
		$Afields=array('prestats'=>$_POST['prestats'], 'preuserid'=>$_SESSION['auser_id'],'prename'=>$_SESSION[$un_auser_name],'predescs'=>$_POST['predescs'],'predate'=>date("Y-m-d H:i:s")); 
    ...
    <TD align=left > 
		  ...
		  <TR>
			...
			<TD>
				...
				&nbsp;&nbsp;<b>金额:</b> <span style="color:#0000ff"><?php echo ($work["id"] > 0)?$work["premoney"].'元':'';?></span> 
			</TD>
		  </TR>
		  <TR>
		  ...
		  <TR>
			...
			<?php } else {?>
						<INPUT TYPE="radio" NAME="prestats" value="1" <?php echo $work["prestats"]==1?"checked":'';?>>不通过	
						<INPUT TYPE="radio" NAME="prestats" value="2" <?php echo $work["prestats"]==2?"checked":'';?>>通过
						<INPUT TYPE="radio" NAME="prestats" value="0" <?php echo $work["prestats"]<1?"checked":'';?>>不处理
			<?php } ?>
			</TD>
		  </TR>  


			<?php if($work["shennwstats"] < 1){ ?>		  
		  <TR>
			<TD width="120" align="right"><b>备注说明:</b></TD>
			<TD>

		    	  <TEXTAREA NAME="predescs" style="height:40px;width:400px;background-color: #FFFFD0"><?php echo ($work["id"] > 0)?$work["predescs"]:'';?></TEXTAREA>
			</TD>
		  </TR> 
		  <TR>
