...
class cls_image
{
    ...
    function upload_image($upload, $dir = '', $img_name = '')
    {
        ...
        if (!file_exists($dir))
        {
            ...
            {
                /* 创建目录失败 */
                $this->error_msg = sprintf('为只读目录，不能创建', $dir);
        ...
        {
            $this->error_msg = sprintf('文件上传失败', $upload['name']);
     ...
     */
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='',$alowbg=true)
    {
        ...
        /* 将原始图片进行缩放处理 imagecreatetruecolor */
        if ($gd == 2)
        {
        ...
        if (!file_exists($dir))
        {
            ...
            {
                /* 创建目录失败 */
                $this->error_msg  = sprintf('创建目录失败,为只读目录', $dir);
                $this->error_no   = ERR_DIRECTORY_READONLY;
                return false;
            }
        }
    ...
    function check_file_type($img_type) {
        return $img_type == 'image/pjpeg' ||
				...
				$img_type == 'audio/wav'  || 
				$img_type == 'audio/mid'  ||  
				$img_type == 'application/vnd.rn-realmedia-vbr'  ||
				$img_type == 'application/x-zip-compressed' || 
				$img_type == 'application/x-shockwave-flash' ||
				$img_type == 'application/octet-stream'  ||
				$img_type == 'application/vnd.ms-excel'  ||
				$img_type == 'application/vnd.ms-powerpoint'  || 
    ...
    function unique_name($dir)
    {
        ...
        while (empty($filename))
        {
            ...
            if (file_exists($dir . $filename . '.jpg') || file_exists($dir . $filename . '.gif') || file_exists($dir . $filename . '.png') || file_exists($dir . $filename . '.txt') || file_exists($dir . $filename . '.doc')|| file_exists($dir . $filename . '.ppt')|| file_exists($dir . $filename . '.zip'))
            {
                $filename = '';
            }
        }
