...
if (!$smarty->is_cached($Aconf["displayFile"], $cache_id)) {
	...
	/* 关联产品 */
	/* 责任编辑 */
	$sql = "SELECT b.user_name FROM ".$pre."admin_user  as b WHERE  b.user_id = '".$rowarticle[user_id]."'";
	$rowarticle[user_id] = $oPub->getOne($sql);

	//新闻主体结束 
	/* 当前位置导航 */ 
	$sql = "SELECT name,fid FROM ".$pre."articat 
		where  acid = '".$rowarticle[acid]."'
	...
	$Ahome["nowNave"]  = '<li><A HREF="./">'.$Aweb_url["index"][0].'</A></li><li><A HREF="'.$articles.'">'.$Aweb_url["articles"][0].'</a></li>'.$strPrenave.'<li>'.$nowcatname.'</li><li><span style="font-weight:lighter"> 正文</span></li>'; 
