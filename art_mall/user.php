...
$firstname     = !empty($firstname)?clean_html($firstname):'';
$lastname     = !empty($lastname)?clean_html($lastname):'';
...
if($o == 'r' && !empty($user_name) && !empty($email) && !empty($password))
{
	...
	if(strtoupper($_SESSION['vCode']) != strtoupper($vcode) || empty($vcode)){ 
		 echo "<SCRIPT language='javascript'>\nalert('the code is wrong 验证码错误!!');top.location='user.php?o=r';</script>";
		 exit;		
	} 
	...
	{ 
		$strMessage = 'register failed,username or password can not be empty.注册失败：用户名或者密码不能为空！';
	}
	elseif($password != $re_password)
	{
		$strMessage = 'register failed, the password is not same.注册失败：两次输入的密码不同！';
	} elseif(str_len($user_name) <2 || str_len($user_name) > 26)
	{
		$strMessage = 'register failed,注册失败：用户名应该为，4-26个字符';
	}elseif(!is_email($email))
	{
		$strMessage = 'register failed,the email is wrong注册失败：邮箱地址错误';
	}elseif($lastname== '' or $firstname == '')
	{
		$strMessage = 'register failed,lastname or firstname can not be empty 注册失败：姓名不能为空！';
	} else
	{
		$strMessage =  '';
	} 
	...
	{ 
		$utid = $oPub->getOne('SELECT id  FROM '.$pre.'userstype where domain_id="'.$Aconf['domain_id'].'" order by  orders asc  limit 1'); 
 		$Afields=array('utid'=>$utid,'user_name'=>$user_name,'lastname'=>$lastname,'firstname'=>$firstname,'password'=>$password,'email'=>$email,'reg_time'=>gmtime(),'domain_id'=>$Aconf['domain_id']);
		...
		$userstr = '&nbsp;&nbsp;&nbsp;&nbsp; <b>'.$user_name.' register successful 成功注册：</b><br/><br/>';
		$userstr.= '&nbsp;&nbsp;&nbsp;&nbsp; <A HREF="user.php?o=d">进入用户中心 user center >>></A> <br/><br/>';
		$userstr.= '&nbsp;&nbsp;&nbsp;&nbsp; <A HREF="./">返回网站首页 homepage >>></A> ';
		$userstr.= '<script type="text/javascript">
function show(){
var i=3;
var div=document.createElement("div");
div.style.position="absolute";
div.style.left="333px";
div.style.top="100px";
div.style.height="400px";
div.style.width="700px";
div.style.opacity=1;
div.style.filter="alpha(opacity=100)";
div.style.backgroundColor="#94C2E0";
div.dir="rtl";
var button=document.createElement("input");
button.type="button";
button.value="X";
button.onclick=function(){remove(div)};
div.appendChild(button);
var divin=document.createElement("div");

var center=document.createElement("center");
center.appendChild(divin);
...
function shownum(divin,i,div){
if(i==-1){
remove(div);
}
divin.innerHTML="&nbsp;&nbsp;&nbsp;&nbsp;thanks for registering, you will receive a comformation in your email<br/><br/>感谢注册，你将收到一封确认邮件<br/><br/>this windows will be closed in "+i+"seconds<br/><br/>"+i+"秒后将自动关闭窗口"

window.setTimeout(function(){shownum(divin,i-1,div)},1000);
}
